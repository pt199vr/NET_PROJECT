        ...
        while True:
            ...
            if len(contours) < 50:
                for i, contour in enumerate(contours):
                     ...
                     #if i < 1:    
                         if cv2.contourArea(contour) > 0:  # Adjust this value based on insect size
                            (x, y, w, h) = cv2.boundingRect(contour)
                            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                            cv2.putText(frame, "Insect Detected", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                            
                            #timestamp = time.strftime("%Y_%m_%d__%H_%M_%S_%f")
                            timestamp = datetime.now().strftime('%Y_%m_%d_%H_%M_%S_%f')[:-3]  # Format to milliseconds
        
                            cropped_image = frame_raw[y:y+h, x:x+w]
